...
pub(crate) fn euler11() -> usize {
    let grid = {
        ...
        01 70 54 71 83 51 54 69 16 92 33 48 61 43 52 01 89 19 67 48".split('\n').collect::<Vec<&str>>().iter().map(|&line| {
            let temp: Vec<usize> = line.split(' ').filter(|&v| v.len() != 0).map(|v| v.parse::<usize>().unwrap()).collect::<Vec<usize>>();
            temp
    ...
    for i in 0..grid[0].len() {
        for _j in 0..grid.len() {
            let j = _j as isize;
            let test_y = grid[i][_j] *
                try_index_else_one(&grid, i, j + 1) *
                try_index_else_one(&grid, i, j + 2) *
                try_index_else_one(&grid, i, j + 3);
            let test_x = grid[i][_j] *
                try_index_else_one(&grid, i + 1, j) *
                try_index_else_one(&grid, i + 2, j) *
                try_index_else_one(&grid, i + 3, j);
            let test_dp = grid[i][_j] *
                try_index_else_one(&grid, i + 1, j + 1) *
                try_index_else_one(&grid, i + 2, j + 2) *
                try_index_else_one(&grid, i + 3, j + 3);
            let test_dn = grid[i][_j] *
                try_index_else_one(&grid, i + 1, j - 1) *
                try_index_else_one(&grid, i + 2, j - 2) *
                try_index_else_one(&grid, i + 3, j - 3);

            let test = test_x.max(test_y).max(test_dp).max(test_dn);
            if test > product {
                product = test;
            }
        }
...
fn try_index_else_one(grid: &Vec<Vec<usize>>, i: usize, j: isize) -> usize {
    if i >= grid.len() || j < 0 || j >= grid[0].len() as isize {
        return 1usize;
    }
...
pub(crate) fn euler12() -> usize {
    ...
    loop {
        ...
        if find_factors(triangle).len() > 500 {
            break;
        }
    }
...
fn find_factors(num: usize) -> Vec<usize> {
    ...
    let mut i = 1;
    while (i as f32) < (num as f32).sqrt() {
        if num % i == 0 {
            factors.push(i);
            if num / i != i {
                factors.push(num / i);
...
pub(crate) fn euler13() -> String {
    ...
    let mut carry = 0u8;
    let mut total = [0u8; 10];

    for i in (0..50).rev() {
        total[..].rotate_right(1);
        let mut sum = 0u16;
        for j in 0..100 {
            sum += numbers[j][i] as u16;
        }
        sum += carry as u16;
        total[0] = (sum % 10) as u8;
        carry = (sum / 10) as u8;
    }
    ...
    total.iter().for_each(|digit| output.push((digit + 48u8) as char));

    output

}

pub(crate) fn euler14() -> u32 {
